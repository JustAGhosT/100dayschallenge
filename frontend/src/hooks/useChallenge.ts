...
import type { Project, Challenge, UserProfile, ChallengeState } from '@/types';

export function useChallenge(userId: string | undefined) {
  const { toast } = useToast();
  const [projects, setProjects] = useState<Project[]>([]);
  const [activeChallenge, setActiveChallenge] = useState<(ChallengeState & Challenge) | null>(null);
  ...
  const fetchProjects = useCallback(async (challengeId: string) => {
  ...
  const addProject = async (projectData: Omit<Project, 'id' | 'user_id' | 'challenge_id' | 'created_at' | 'status'>) => {
